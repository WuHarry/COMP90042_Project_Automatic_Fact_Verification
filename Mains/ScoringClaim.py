...
class ScoringClaim:
    def __init__(self, glove_path, train_data_path):
        super(ScoringClaim, self).__init__()

        word2emb = build_index_from_glove(glove_path)

        train_pickle = read_train_set(train_data_path)

        claim_sequences, evidence_sequences, labels, word_index = preprocess_train_data(train_pickle)
        with open('score_word_index.json', 'w') as outfile:
    ...
    def train(self):
        ...
        self.model.fit([self.train_data_claim, self.train_data_evidence], self.train_label, validation_data=([self.vali_data_claim, self.vali_data_evidence], self.vali_label), epochs=30, batch_size=600, callbacks=callbacks_list, verbose=1)

        self.model.save('scoring_model.h5')
...
if __name__ == '__main__':
    ...
    sc = ScoringClaim(glove_path, train_data_path)
    sc.train()
